...
LUA_SHA1 = get_hash(os.path.join(CWD, 'tindeck.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            '--warc-header', 'tindeck-dld-script-version: ' + VERSION,
            '--warc-header', ItemInterpolation('tindeck-item: %(item_name)s'),
        ]
...
project = Project(
    title = 'tindeck',
    project_html = '''
    <img class="project-logo" alt="logo" src="https://www.archiveteam.org/images/2/24/Tindeck_logo.png" height="50px"/>
    <h2>tindeck.com <span class="links"><a href="https://tindeck.com/">Website</a> &middot; <a href="http://tracker.archiveteam.org/tindeck/">Leaderboard</a></span></h2>
    '''
)
...
pipeline = Pipeline(
    ...
    GetItemFromTracker('http://%s/%s' % (TRACKER_HOST, TRACKER_ID), downloader,
        VERSION),
    PrepareDirectories(warc_prefix='tindeck'),
    WgetDownload(
        WgetArgs(),
        max_tries=2,
