...
More information about the archiving project can be found on the ArchiveTeam wiki: [tindeck](http://archiveteam.org/index.php?title=tindeck)
...
Follow the [instructions on the ArchiveTeam wiki](http://archiveteam.org/index.php?title=Warrior) for installing the Warrior, and select the "tindeck" project in the Warrior interface.
...
### For Debian/Ubuntu:
    ...
    pip install --upgrade seesaw
    su -c "cd /home/archiveteam; git clone https://github.com/ArchiveTeam/tindeck-grab.git; cd tindeck-grab; ./get-wget-lua.sh" archiveteam
    screen su -c "cd /home/archiveteam/tindeck-grab/; run-pipeline pipeline.py --concurrent 2 --address '127.0.0.1' YOURNICKHERE" archiveteam
    [... ctrl+A D to detach ...]
...
**There is a known issue with some packaged versions of rsync. If you get errors during the upload stage, tindeck-grab will not work with your rsync version.**
...
6. `screen su -c "cd /home/archiveteam/tindeck-grab/; run-pipeline pipeline.py --concurrent 2 --address '127.0.0.1' YOURNICKHERE" archiveteam`
...
Have an issue not listed here? Join us on IRC and ask! We can be found at irc.efnet.org #findeck.
